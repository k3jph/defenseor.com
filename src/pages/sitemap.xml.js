...
export function GET() {
  const paths = ['/', '/chapters/', '/contributors/', '/reading-room/', '/book/', '/book/first-edition/', '/notices/', '/about/', '/contact/', '/chapters/1e/', ...chapters.map(c => c.url), ...contributors.map(p => `/contributors/${p.slug}/`), ...pathways.map(p => `/reading-room/${p.slug}/`)];
